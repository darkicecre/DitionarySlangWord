...
public class Feature3_3 extends JFrame{
	private int heart = 3;
	private Word dictionary;
	private int keyQues=0;
	private	ImageIcon background1_1=new ImageIcon("image\\background3_3.PNG");
	private ImageIcon nullFrame=new ImageIcon("image\\3_3_button.PNG");
	...
	private	Icon t=new ImageIcon("image\\3_3_home.PNG");
	private Icon c=new ImageIcon("image\\3_3_continue.PNG");
	...
	private void prepareLabel(JLabel label){
		...
		Color myBlue = new Color(96,128,68);
	...
	public Feature3_3(Word dictionary){
		ReadFile data = new ReadFile("dtb\\scoreDefinition.txt");
		data.writeString("0");
		prepareLabel(score);
		...
		button_t.setActionCommand("3_3_t");
		button_c.setBounds(40,20,470,118);
		button_c.setActionCommand("3_3_c");
	...
	public void resetScore(){
		...
		if(a>b){
			b=a;
			ReadFile dataHigh=new ReadFile("dtb\\highscoreDefinition.txt");
	...
	public void handle(){
		ReadFile high = new ReadFile("dtb\\highscoreDefinition.txt");
		ReadFile scoreMine=new ReadFile("dtb\\scoreDefinition.txt");
		highScore.setText(high.readString());
		score.setText(scoreMine.readString());
		String[] key=dictionary.guessDefinition();
		prepareLabel(ques);
		prepareLabel(ques1);
		...
		//button4.setText("<html>tip line one<br>tip line two</html>");
		ques.setText(key[4]);
		ques1.setText("");
		if(key[4].length()>=26){
			int i=0;
			...
			if(str=="@"){
				...
				if(Integer.parseInt(str)==keyQues){
					...
					if(int_score>Integer.parseInt(highScore.getText())){
						...
						ReadFile highdata = new ReadFile("dtb\\highscoreDefinition.txt");
						highdata.writeString(highScore.getText());
					}
					score.setText(""+int_score);
					ReadFile write =new ReadFile("dtb\\scoreDefinition.txt");
